...
describe("analyzeBimi", () => {
  it("returns warn when no BIMI record found", async () => {
    ...
    expect(result.validations.some((v) => v.message.includes("No BIMI record"))).toBe(true);
  });

  it("shows DMARC policy pass when no record but policy is reject", async () => {
    mockQueryTxt.mockResolvedValue(null);
    const result = await analyzeBimi("example.com", "reject");
    expect(result.validations.some((v) => v.status === "pass" && v.message.includes("DMARC policy meets BIMI requirement"))).toBe(true);
  });
  ...
  it("warns about DMARC policy when no record and policy is none", async () => {
    ...
    expect(result.validations.some((v) => v.status === "warn" && v.message.includes("BIMI requires"))).toBe(true);
  });
  ...
  it("parses valid BIMI record with logo and authority", async () => {
    mockQueryTxt.mockResolvedValue({
      entries: ["v=BIMI1; l=https://example.com/logo.svg; a=https://example.com/vmc.pem"],
    ...
    expect(result.validations.some((v) => v.message.includes("BIMI record found"))).toBe(true);
    expect(result.validations.some((v) => v.message.includes("Logo URL") && v.message.includes("HTTPS"))).toBe(true);
    expect(result.validations.some((v) => v.message.includes("Authority evidence"))).toBe(true);
  });
  ...
  it("warns when logo URL does not use HTTPS", async () => {
    ...
    const result = await analyzeBimi("example.com", "reject");
    expect(result.validations.some((v) => v.status === "warn" && v.message.includes("should use HTTPS"))).toBe(true);
  });
  ...
  it("warns when no logo URL specified", async () => {
    ...
    const result = await analyzeBimi("example.com", "reject");
    expect(result.validations.some((v) => v.status === "warn" && v.message.includes("No logo URL"))).toBe(true);
  });
  ...
  it("fails when DMARC policy is none with BIMI record present", async () => {
    ...
    const result = await analyzeBimi("example.com", "none");
    expect(result.validations.some((v) => v.status === "fail" && v.message.includes("DMARC policy must be"))).toBe(true);
  });
  ...
  it("handles TXT record that exists but is not valid BIMI", async () => {
    ...
    const result = await analyzeBimi("example.com", "reject");
    expect(result.status).toBe("warn");
    expect(result.validations.some((v) => v.message.includes("not a valid BIMI record"))).toBe(true);
  });
